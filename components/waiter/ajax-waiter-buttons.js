...
function initOrderForm() {
    ...
    function addDishToOrder() {
        ...
        dishElement.innerHTML = `
            <span class="form__info-item">${dishName}</span>
            <span class="form__info-item">${quantity} x ${dishPrice}₽</span>
...
async function handleFormSubmit(form) {
    try {
        ...
        if (contentType?.includes('application/json')) {
            ...
            // Обработка УСПЕШНОГО изменения статуса
            if (form.id === 'change-status-form' && result.success) {
                // Обновляем статус в таблице
                const statusCell = document.querySelector(`[data-order-id="${result.order_id}"]`);
                if (statusCell) statusCell.textContent = result.new_status;

                showNotification(`Статус изменен: ${result.new_status}`, 'success');
                closeModal();
                return; // Прерываем выполнение
            }
        }

        closeModal(); // Закрываем для других форм
    } catch (error) {
